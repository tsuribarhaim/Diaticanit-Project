...
export function TargetsWorkspace({
  ...
  // The page no longer generates a baseline plan itself before rendering
  // (that used to block the whole page's server render on an AI call that
  // routinely takes 30-90s - see targets/page.tsx's own comment on why).
  // Instead, for a first-time user with nothing to show yet, this fires the
  // exact same generation as clicking "Generate my targets" automatically,
  // once, right after the page has already rendered - so the page itself
  // shows up immediately, and only the plan preview below waits on the AI
  // call, with its own pending state (see the "Generating your baseline
  // plan" banner below and GenerateSubmitButton's own pending label).
  useEffect(() => {
    if (mode !== "initial" || initialPreview || hasAutoGeneratedRef.current) return;
  ...
  return (
    <div className="space-y-4">
      ...
      {mode === "initial" && !displayedPayload && isGeneratePending ? (
        <div className="flex items-center gap-3 rounded-xl border border-teal-200 bg-teal-50/40 px-4 py-3 text-sm text-teal-900 dark:border-teal-800 dark:bg-teal-950/30 dark:text-teal-300">
          ...
          <p>
            {tr(
              locale,
              "Generating your personalized baseline plan from your profile - this can take up to a minute.",
              "יוצרים עבורך תכנית בסיס מותאמת אישית מתוך הפרופיל שלך - זה עשוי לקחת עד דקה.",
            )}
          </p>
        </div>
      ) : null}
      ...
      <div>
        ...
        <form id={GENERATE_FORM_ID} ref={generateFormRef} action={generateFormAction} className="mt-2 space-y-3">
          <textarea
            name="goal_text"
            maxLength={500}
